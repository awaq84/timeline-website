...
async function main() {
  ...
  for (const [title, question] of Object.entries(HAND_WRITTEN)) {
    ...
    chosen.push({ title: e.title, year: e.year, question });
  ...
  for (const e of events) {
    ...
    candidates.push({ title: e.title, year: e.year, question, category: e.category, fame, key });
  }
  ...
  for (const c of candidates) {
    ...
    seenQuestions.add(q);
    chosen.push({ title: c.title, year: c.year, question: c.question });
  }
