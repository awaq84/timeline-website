...
const quizDifficultyHintEl = document.getElementById("quizDifficultyHint");
...
// ---- Quiz ----
//
// Four statements, a period, and the question of which two fall inside it.
//
// The pool is data/quiz.js (generated by scripts/build-quiz.mjs), which is the
// only place the rules about what makes a fair question live -- exact dates,
// titles that state what happened, nothing self-answering. By the time an entry
// reaches this file it is already safe to grade against. Entries are
// { y year, q statement, c category index, s subject key }.
//
// The period is always shown. That is what makes the question answerable rather
// than a guess: without it "which two happened in the same period" has no fixed
// answer, since any two events share *some* 25-year window.

const QUIZ_SPANS = [1, 5, 15, 25];
const QUIZ_HINTS = {
  1: "Hardest — both events fall in one exact year.",
  5: "Hard — both events fall inside the same 5 years.",
  15: "Medium — both events fall inside the same 15 years.",
  25: "Easiest — both events fall inside the same 25 years.",
};

// Score is deliberately a plain variable. /privacy/ promises the site sets no
// cookies and writes nothing to local or session storage, so the run has to end
// when the tab does.
const quiz = {
  loaded: false,
  loading: false,
  pool: [], // sorted by year
  years: [], // pool.map(p => p.y), for binary search
  span: 1,
  puzzle: null,
...
function lowerBound(target) {
  let lo = 0;
  let hi = quiz.years.length;
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    if (quiz.years[mid] < target) lo = mid + 1;
    else hi = mid;
  }
  return lo;
}

function quizRange(lo, hi) {
  return quiz.pool.slice(lowerBound(lo), lowerBound(hi + 1));
}
...
// span, which is what makes 1 year genuinely harder than 25 rather than just
// differently worded.
function buildPuzzle(span) {
  if (!quiz.pool.length) return null;
  const gap = Math.max(1, Math.round(span / 2));
  const reach = Math.max(20, span * 5);

  for (let attempt = 0; attempt < 600; attempt++) {
    const anchor = quiz.pool[randInt(quiz.pool.length)];
    ...
    const inside = quizRange(start, end);
    if (inside.length < 2) continue;

    const outside = quizRange(start - reach, start - gap).concat(quizRange(end + gap, end + reach));
...
function renderQuiz() {
  ...
  const p = quiz.puzzle;
  if (!p) {
    // Never seen in 80,000 generated puzzles, but "try another" with nothing to
    // click would be a dead end, and the difficulty pills ignore a click on the
    // setting that is already active -- so the retry has to be its own button.
    quizBodyEl.innerHTML = "";
    const msg = document.createElement("p");
    msg.className = "quiz-loading";
    msg.textContent = "Couldn't put a question together for that period.";
    const retry = document.createElement("button");
    retry.type = "button";
    retry.className = "icon-btn-accent quiz-primary";
    retry.textContent = "Try again";
    retry.addEventListener("click", () => newQuizQuestion());
    quizBodyEl.append(msg, retry);
    return;
  }
  ...
  period.className = "quiz-period";
  period.textContent = quizPeriodLabel(p);
  quizBodyEl.appendChild(period);
  ...
  p.options.forEach((opt, i) => {
    ...
    if (quiz.picked.includes(i)) btn.classList.add("is-picked");
    btn.setAttribute("aria-pressed", quiz.picked.includes(i) ? "true" : "false");

    const dot = document.createElement("span");
    dot.className = "quiz-dot";
    dot.style.background = CATEGORY_COLORS[CATEGORY_ORDER[opt.c]] || "#8a8a8e";
    btn.appendChild(dot);
    ...
    text.textContent = opt.q.charAt(0).toUpperCase() + opt.q.slice(1);
    btn.appendChild(text);

    if (quiz.graded) {
      btn.disabled = true;
      const isAnswer = p.answer.has(i);
      const picked = quiz.picked.includes(i);
      ...
      year.className = "quiz-option-year";
      year.textContent = formatYear(opt.y);
      btn.appendChild(year);
    } else {
      btn.addEventListener("click", () => pickQuizOption(i));
    }

    list.appendChild(btn);
  });
  quizBodyEl.appendChild(list);
  ...
  primary.type = "button";
  primary.className = "icon-btn-accent quiz-primary";
  if (quiz.graded) {
    primary.textContent = "Next question";
    primary.addEventListener("click", () => newQuizQuestion());
  } else {
    primary.textContent = "Check answer";
    primary.disabled = quiz.picked.length !== 2;
    primary.addEventListener("click", () => gradeQuiz());
  }
  ...
  if (quiz.graded) {
    ...
    // guess did instead of restating them. Listing the correct years here read
    // as broken at the 1-year setting, where it printed the same number twice.
    const hits = quiz.picked.filter((i) => p.answer.has(i)).length;
    feedback.classList.add(hits === 2 ? "is-right" : "is-wrong");
    feedback.textContent =
      hits === 2
        ? "Correct — both of those fall inside the period."
        : hits === 1
        ? "Half right — one of your two falls inside the period."
        : "Not quite — neither of your picks falls inside the period.";
  } else {
    feedback.textContent = quiz.picked.length === 2 ? "" : `Pick ${2 - quiz.picked.length} more.`;
  }
  ...
  if (quiz.asked) {
    ...
    score.textContent = `Score: ${quiz.right} / ${quiz.asked}`;
    actions.appendChild(score);
  }

  quizBodyEl.appendChild(actions);
}
...
function pickQuizOption(i) {
  const at = quiz.picked.indexOf(i);
  if (at >= 0) quiz.picked.splice(at, 1);
  else {
    quiz.picked.push(i);
    if (quiz.picked.length > 2) quiz.picked.shift();
  }
  renderQuiz();
}

function gradeQuiz() {
  if (quiz.graded || quiz.picked.length !== 2) return;
  quiz.graded = true;
  quiz.asked++;
  if (quiz.picked.every((i) => quiz.puzzle.answer.has(i))) quiz.right++;
  renderQuiz();
}

function newQuizQuestion() {
  quiz.puzzle = buildPuzzle(quiz.span);
  quiz.picked = [];
  quiz.graded = false;
  renderQuiz();
}

// data/quiz.js is 228KB and most visitors are here for the map, so it is fetched
// the first time the section comes into view rather than on load.
function loadQuizPool() {
  if (quiz.loaded || quiz.loading) return;
  quiz.loading = true;
  const s = document.createElement("script");
  s.src = "data/quiz.js?v=1";
  s.onload = () => {
    quiz.loading = false;
    if (typeof QUIZ_EVENTS === "undefined" || !QUIZ_EVENTS.length) {
      quizSectionEl?.setAttribute("hidden", "");
      return;
    }
    quiz.pool = QUIZ_EVENTS.slice().sort((a, b) => a.y - b.y);
    quiz.years = quiz.pool.map((p) => p.y);
    quiz.loaded = true;
    newQuizQuestion();
  };
  s.onerror = () => {
    quiz.loading = false;
    if (quizBodyEl) {
      quizBodyEl.innerHTML = `<p class="quiz-loading">Couldn't load the questions. Please reload the page.</p>`;
    }
  };
...
function initQuiz() {
  if (!quizSectionEl || !quizBodyEl) return;

  quizDifficultyEl?.addEventListener("click", (ev) => {
    const btn = ev.target.closest("button[data-span]");
    if (!btn) return;
    const span = Number(btn.dataset.span);
    if (!QUIZ_SPANS.includes(span) || span === quiz.span) return;
    quiz.span = span;
    quizDifficultyEl.querySelectorAll("button").forEach((b) => b.classList.toggle("is-active", b === btn));
    if (quizDifficultyHintEl) quizDifficultyHintEl.textContent = QUIZ_HINTS[span];
    if (quiz.loaded) newQuizQuestion();
  });
  ...
  // is a far worse outcome than the download it was avoiding. So the pool is
  // also fetched once the page is otherwise idle.
  //
  // The saving was never the point anyway: 48KB gzipped is smaller than
  // discover.js, which this page already loads eagerly in a blocking tag.
  // loadQuizPool() is idempotent, so whichever path gets there first wins.
  const idle = () =>
    (window.requestIdleCallback || ((fn) => setTimeout(fn, 1200)))(loadQuizPool, { timeout: 4000 });
...
async function init() {
  // The quiz has its own data file and reads nothing from INDEX, so it is
  // started before the await rather than after it. Everything below returns
  // early if the index fails to load, and there is no reason for a map outage
  // to take the quiz down with it.
  initQuiz();
